...
def get_node_attention(raw_attention_value, validation_num_list, DG_graph_list, DG_list_for_reaction):
    
    zero_indices = torch.where(raw_attention_value == 0)[0]
    split_sizes = [zero_indices[0]+1]
    for n in range(len(zero_indices)-1):
        split_size = zero_indices[n+1] - zero_indices[n]
        split_sizes.append(split_size)
    sub_tensors = torch.split(raw_attention_value, split_sizes)
    
    total_edges = 0
    for i in validation_num_list:
        edges =  DG_graph_list[i].edges()
        total_edges += len(edges[0])
        sub_tensor = sub_tensors[0][total_edges - len(edges[0]):total_edges]
        num_atom = DG_list_for_reaction[i].GetNumAtoms()
        attention_aggr = [0] * num_atom
        for j in range(len(edges[0])):
            attention_aggr[edges[0][j]] += sub_tensor[j]
            attention_aggr[edges[1][j]] += sub_tensor[j]
            degree = []
            for k, atom in enumerate(DG_list_for_reaction[i].GetAtoms()):
                b=atom.GetDegree()
                degree.append(b)
            if j == len(edges[0]) - 1:
                ...
                for k, atom in enumerate(DG_list_for_reaction[i].GetAtoms()):
                    ...
                    if c != 1:
                        print(k+1, atten_aver)
                print("Total number of atoms is: {:.0f}".format(num_atom))
                
def plot_encoding_space(hidden_feature, hidden_feature_oos, oos_num):
    
    hidden_feature= hidden_feature.detach().numpy()
    hidden_feature_oos= hidden_feature_oos.detach().numpy()
    decs_all_embed = np.concatenate([hidden_feature, hidden_feature_oos],axis=0)
    
    tsne = TSNE(n_components=3, perplexity=2, n_iter=5000)
    tsne_results = tsne.fit_transform(decs_all_embed)
    tsne_train_val = tsne_results[:-oos_num,:]
    tsne_oos = tsne_results[-oos_num:,:]

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(tsne_train_val[:, 0], tsne_train_val[:, 1], tsne_train_val[:, 2], color='blue', label='Training set', alpha=0.5)
    ax.scatter(tsne_oos[:, 0], tsne_oos[:, 1], tsne_oos[:, 2], color='red', label='OOS set', alpha=0.5)
